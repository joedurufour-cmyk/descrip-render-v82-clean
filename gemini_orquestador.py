...
2b. OBLIGATORIO evitar null por defecto: en una fotografía o ilustración normal, la pose/postura (`accion_estado_detectado`), el entorno o fondo aunque sea mínimo (`contexto_detectado`) y el encuadre/distancia/ángulo de cámara (`lente_angulo_detectado`) son prácticamente SIEMPRE determinables — toda imagen tiene una pose corporal, algo detrás del sujeto (aunque sea "fondo liso desenfocado" o "estudio con fondo neutro") y un encuadre (aunque sea "plano medio, cámara a la altura de los ojos"). Dejar estos campos en null es la EXCEPCIÓN, no el default: solo hazlo si la imagen está genuinamente tan recortada/oculta/borrosa que no puedes describir ni siquiera en términos generales — y en ese caso, agrega el campo a `confianza_baja` en vez de simplemente omitirlo. Preservar pose, entorno y encuadre es tan importante como preservar el sujeto: son justamente lo que distingue "la misma escena en otro estilo" de "una escena genérica nueva".
...
3b. SEPARACIÓN ESTRICTA sujeto/rasgos vs. medio: `sujeto_detectado`, `rasgos_fisicos_detectados`, `accion_estado_detectado` y `contexto_detectado` describen SOLO hechos físicos y compositivos (quién/qué es, anatomía, prendas, pose, objetos, entorno) — NUNCA nombres ahí el medio o estilo artístico de renderizado (anime, óleo, foto, 3D, ilustración, manga, acuarela, etc.). Esos términos van EXCLUSIVAMENTE en `medio_estilo_detectado`. Esto es crítico: estos campos se reutilizan sin cambios al regenerar la misma imagen en OTRAS categorías estéticas (fotorrealismo, pintura clásica, cyberpunk...), así que si el estilo original queda incrustado en el sujeto, contamina y contradice cualquier categoría distinta que el usuario elija después.
   Ejemplo INCORRECTO: sujeto_detectado="Mujer joven de estilo anime con cabello verde oscuro corto".
